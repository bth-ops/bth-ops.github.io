---
title: "Övning: Index och prestanda i databas" 
description: "Övning för att se hur olika index funderar i en databas och hur de kan påverka databasens prestanda."
author: mos
revision:
    "2026-03-08": "(A, mos) Första versionen utifrån en befintlig artikel https://dbwebb.se/kunskap/index-och-prestanda-i-mysql."
sidebar:
    order: 0800
---
...
{/*
TODO
* Kolla manualen i Mariadb om den tar upp fler saker.
    * [Building the best INDEX for a given SELECT](https://mariadb.com/docs/server/ha-and-performance/optimization-and-tuning/optimization-and-indexes/building-the-best-index-for-a-given-select)
* Utöka exempel med FK
...
Du har goda kunskaper i SQL och i MariaDB (eller MySQL).
...
Motsvarande information för MariaDB finner du i "[Optimization and Tuning](https://mariadb.com/kb/en/optimization-and-tuning/)". Det är främst delen i "[Optimization and Indexes](https://mariadb.com/docs/server/ha-and-performance/optimization-and-tuning/optimization-and-indexes)" som hanteras i denna artikel.
...
Skapa filen `index.sql` och lägg in följande kommentar, överst i filen.

```sql title="index.sql"
-- Example code on how to work with index.
-- Course: databas
-- Author: Your Name
-- Date: ÅÅÅÅ-MM-DD
```
...
## Skapa egen tabell för tester

Vi behöver en tabell med innehåll för att testa hur index påverkar prestanda av SQL frågor, använd följande.

```sql title="Tabell för att testa index."
...
Tabellen ser ut så här när du gör en SELECT mot den.
...
Spara all kod du skriver och lägger kommentarer kring den, du kan ha nytta av sådana kodexempel framöver när du behöver skapa egna index till dina tabeller.
...
Låt oss undvika delen med hårdvaran och hur databasen är installerad och konfigurerad. Vi väljer istället att fokusera på vilken optimering som kan göras på databasnivå med SQL-frågor och tabeller.
...
Vi kan använda [EXPLAIN](https://mariadb.com/docs/server/reference/sql-statements/administrative-sql-statements/analyze-and-explain-statements/explain) mot tabellen, för att se hur tabellen är strukturerad. Utskriften ger oss tabellens struktur och information om datatyper, NULL, nycklar med mera. 
...
MySQL/MariaDB stödjer något som heter FULLTEXT index vilket tillåter mer avancerad sökning än `LIKE` på större textdokument eller strängar.
...
Låt oss pröva en variant av SELECT och index där jag vill ha fram alla rader som är större än ett numeriskt värde, säg `points > 7.5` för alla kurser som är större än 7.5hp.
...
Däremot försöker databasen att skapa ett index för varje FOREIGN KEY som vi lägger till. Databasen har bestämt sig för att vi troligen kommer vilja joina tabeller på en främmande nyckel och därför väljer den att automatiskt försöka lägga till ett matchande index.
...
En databas är ett levande ting och beter sig olika över tiden, så det är bra att ha koll på denna möjligheten.

Man kan också sätta på loggning i en session man jobbar med, man kan utföra ett antal frågor och sedan visa hur lång tid de tog och hur mycket resurser de använde. Detta styr man med [`SHOW PROFILE`](https://mariadb.com/docs/server/reference/sql-statements/administrative-sql-statements/show/show-profile).
...
Först sätter vi på loggning för profilering.
...
Annars kommer du långt med:

> Fixa index så att du undviker full table scan.



## Avslutningsvis

Du har nu fått en allmän genomgång av aspekter som kan påverka prestandan av din databas och fokus har varit på index. Vill du läsa mer så är manualen för MySQL/MariaDB och den delen som handlar om Optimization, en god punkt att fortsätta.
